...
class RealtimeFallDetector:
    ...
    def __init__(self, model_path, model_type='3class', buffer_size=30):
        ...
        if feature_file.exists():
            with open(feature_file, 'r') as f:
                lines = f.readlines()
                # "1. feature_name" 형식에서 feature_name만 추출
                self.feature_names = []
    ...
    def add_dynamic_features(self, features_list):
        ...
        # 1. 각 keypoint의 속도 및 가속도 (17개 × 6 = 102개)
        for kp_name in self.keypoint_names:
            x_col = f'{kp_name}_x'
            y_col = f'{kp_name}_y'
        ...
        # 4. 가속도 센서 변화 (웹캠에는 없지만 feature 맞추기 위해)
        current['acc_mag_diff'] = 0
        current['acc_mag_mean_5'] = 0
        current['acc_mag_std_5'] = 0
    ...
    def predict(self, features):
        ...
        # 학습 시 사용한 feature 순서에 맞춰 정렬 (CRITICAL!)
        ...
        try:
            ...
            if self.feature_names:
                print(f"   예상 Features: {len(self.feature_names)}")
                print(f"   실제 Features: {len(df.columns)}")
                missing = set(self.feature_names) - set(df.columns)
                if missing:
                    print(f"   누락된 Features: {list(missing)[:5]}...")
