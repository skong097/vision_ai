"""
실시간 낙상 감지 시스템 (웹캠 기반) - 최종 수정 버전
Author: Home Safe Solution Team
Date: 2026-01-28

주요 수정:
- Keypoint 감지 실패 시 예측 건너뛰기 (강화된 검사)
- 상세 로깅
"""
...
class RealtimeFallDetector:
    ...
    def __init__(self, model_path, model_type='3class', buffer_size=30, log_dir='logs'):
        ...
        self.skipped_frames = 0  # 건너뛴 프레임 수
    ...
    def extract_features_from_keypoints(self, keypoints):
        ...
        # 🔥🔥🔥 CRITICAL: 강화된 Keypoint 감지 실패 검사
        # 조건 1: 감지된 keypoint가 너무 적음
        if detected_keypoints < 8:
            self.logger.warning(f"⚠️  Keypoint 부족 ({detected_keypoints}/17) - 예측 건너뛰기")
            return None
        
        # 조건 2: 핵심 높이 Feature가 모두 0 (OR 조건으로 변경!)
        if features['hip_height'] == 0 or features['spine_angle'] == 0:
            self.logger.warning(f"⚠️  핵심 Feature 없음 (hip={features['hip_height']}, spine={features['spine_angle']}) - 예측 건너뛰기")
            return None
        
        # 조건 3: 평균 confidence가 너무 낮음
        if features['avg_confidence'] < 0.3:
            self.logger.warning(f"⚠️  Confidence 낮음 ({features['avg_confidence']:.2f}) - 예측 건너뛰기")
            return None
    ...
    def draw_info(self, frame, prediction, proba, features):
        ...
        if features:
            ...
            cv2.putText(frame, f"Spine Angle: {features.get('spine_angle', 0):.1f}", (20, y_offset),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
            y_offset += 25
            cv2.putText(frame, f"Confidence: {features.get('avg_confidence', 0):.2f}", (20, y_offset),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
        ...
        # 건너뛴 프레임 수
        cv2.putText(frame, f"Skipped: {self.skipped_frames}", (w - 150, 110),
    ...
    def run(self, camera_id=0, save_video=False, output_path='output.mp4'):
        ...
        try:
            ...
            self.logger.info(f"건너뛴 프레임: {self.skipped_frames} ({self.skipped_frames/self.frame_count*100:.1f}%)")
