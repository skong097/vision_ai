"""
RF 모델 재학습 v3 — 좌표 정규화 (해상도 무관)
- 모든 keypoint 좌표를 bbox 기준 0~1 정규화
- 속도/가속도도 정규화된 좌표 기반
- 동영상 단위 train/test split

Usage:
    # 테스트 (소량)
    python train_rf_v3_normalized.py --test
    
    # 전체 실행
    python train_rf_v3_normalized.py
"""
...
def extract_normalized_features(keypoints, prev_kp=None, prev2_kp=None, feature_history=None):
    """
    ⭐ 정규화된 181개 Feature 추출
    - 모든 x, y 좌표를 bbox 기준 0~1로 정규화
    - 해상도에 무관한 feature
    """
    features = {}
    ...
    if np.any(valid):
        ...
        bbox_x_min = np.min(xs)
        bbox_y_min = np.min(ys)
        bbox_w = np.max(xs) - bbox_x_min + 1e-6
        bbox_h = np.max(ys) - bbox_y_min + 1e-6
    else:
        bbox_x_min, bbox_y_min = 0, 0
        bbox_w, bbox_h = 1, 1
    
    # ===== 정규화 함수 =====
    def norm_x(x):
        return float((x - bbox_x_min) / bbox_w)
    
    def norm_y(y):
        return float((y - bbox_y_min) / bbox_h)
    
    # prev keypoints도 정규화 (현재 bbox 기준)
    def norm_prev_kp(kp):
        if kp is None:
            return None
        normed = np.zeros_like(kp)
        for i in range(len(kp)):
            normed[i][0] = (kp[i][0] - bbox_x_min) / bbox_w
            normed[i][1] = (kp[i][1] - bbox_y_min) / bbox_h
            normed[i][2] = kp[i][2]
        return normed
    ...
    for i in range(17):
        kp_norm[i][0] = norm_x(keypoints[i][0])
        kp_norm[i][1] = norm_y(keypoints[i][1])
        kp_norm[i][2] = keypoints[i][2]
    
    prev_norm = norm_prev_kp(prev_kp)
    prev2_norm = norm_prev_kp(prev2_kp)
    
    # ===== 1~51: 정규화된 keypoint x, y, conf =====
    ...
    # ===== 61~68: 정규화된 높이/bbox =====
    hip_mid_norm = (kp_norm[11][:2] + kp_norm[12][:2]) / 2
    shoulder_mid_norm = (kp_norm[5][:2] + kp_norm[6][:2]) / 2
    
    features['hip_height'] = float(hip_mid_norm[1])
    features['shoulder_height'] = float(shoulder_mid_norm[1])
    features['head_height'] = float(kp_norm[0][1])
    
    features['bbox_width'] = float(bbox_w / (bbox_w + bbox_h))   # 비율
    features['bbox_height'] = float(bbox_h / (bbox_w + bbox_h))  # 비율
    ...
    for i, name in enumerate(KP_NAMES):
        ...
        if prev2_norm is not None and prev_norm is not None:
    ...
    while True:
        ...
        if len(results) > 0 and results[0].keypoints is not None:
            ...
            if len(keypoints_all) > 0:
                ...
                prev_kp = keypoints.copy()  # 원본 좌표 저장 (다음 프레임 정규화에 사용)
...
def preprocess_all(test_mode=False):
    ...
    for folder_name, info in folders.items():
        ...
        if all_data:
            ...
            print(f"✅ {folder_name}: {len(df)}행, {len(df.columns)}열 → {path}")
...
def train_model():
    ...
    for name, bal_method, bal_ratio, rf_params in strategies:
        ...
        if bal_method == 'undersample':
            n_fallen = sum(train_df['label'] == 1)
            normal_sampled = train_df[train_df['label']==0].sample(
                n=min(n_fallen * bal_ratio, sum(train_df['label']==0)), random_state=42)
            bal_df = pd.concat([normal_sampled, train_df[train_df['label']==1]])
        elif bal_method == 'hybrid':
            n_fallen = sum(train_df['label'] == 1)
            normal_sampled = train_df[train_df['label']==0].sample(
                n=min(n_fallen * bal_ratio, sum(train_df['label']==0)), random_state=42)
            fallen_over = train_df[train_df['label']==1].sample(n=n_fallen*2, replace=True, random_state=42)
            bal_df = pd.concat([normal_sampled, fallen_over])
        else:
            bal_df = train_df
        ...
        fallen_idx = list(rf.classes_).index(1)
        y_proba = rf.predict_proba(X_test)[:, fallen_idx]
    ...
    # 최적 모델 저장
    best = max(results, key=lambda r: (r['f1'], r['rec']))
    print(f"\n🏆 최적: {best['name']} (F1={best['f1']:.1f}%, Recall={best['rec']:.1f}%)")
...
def main():
    ...
    parser.add_argument('--test', action='store_true', help='소량 테스트')
    parser.add_argument('--train-only', action='store_true', help='전처리 건너뛰고 학습만')
