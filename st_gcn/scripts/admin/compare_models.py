...
class Graph:
    def __init__(self):
        ...
        A = np.zeros((1, num_node, num_node), dtype=np.float32)
        for i, j in edges:
            A[0, i, j] = 1
            A[0, j, i] = 1
...
class _STGCNOriginalModel(nn.Module):
    """st_gcn_networks 레이어 이름 사용"""
    def __init__(self, A, num_classes=2):
        super().__init__()
...
class STGCNOriginalPipeline(ModelPipeline):
    ...
    - val_data.npy (N, 3, 60, 17, 1) 시퀀스 입력
    ...
    def load_model(self):
        A = Graph().A
        self.model = _STGCNOriginalModel(A, num_classes=2)
    ...
    def load_test_data(self):
        data_path = ST_GCN_DIR / "data/binary/val_data.npy"
        label_path = ST_GCN_DIR / "data/binary/val_labels.npy"
        self.data_description = f"ST-GCN val_data.npy (60-frame sequences)"
...
# ============================================================
# 3. ST-GCN Fine-tuned 파이프라인
# ============================================================

class _STGCNFineTunedModel(nn.Module):
    """layers 레이어 이름 + data_bn 사용"""
    def __init__(self, A, num_classes=2):
        super().__init__()
...
class STGCNFineTunedPipeline(ModelPipeline):
    ...
    def load_model(self):
        A = Graph().A
        self.model = _STGCNFineTunedModel(A, num_classes=2)
...
def plot_inference_time(results, save_path):
    ...
    names = [f"{r['icon']} {r['short']}" for r in results]
...
def plot_model_size(results, save_path):
    ...
    if dl_results:
        dl_names = [f"{r['icon']} {r['short']}" for r in dl_results]
        dl_params = []
        for r in dl_results:
            ps = r["total_params"]
