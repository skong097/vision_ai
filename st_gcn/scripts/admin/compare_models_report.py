...
class STGCNBlock(nn.Module):
    def __init__(self, in_ch, out_ch, A, stride=1, residual=True):
        super().__init__()
        n_part = A.shape[0]  # 1 or 3
        self.register_buffer('A', torch.tensor(A, dtype=torch.float32))
        self.gcn = nn.Sequential(
            nn.Conv2d(in_ch, out_ch * n_part, 1),
            nn.BatchNorm2d(out_ch * n_part),
        )
        self.n_part = n_part
        self.out_ch = out_ch
    ...
    def forward(self, x):
        res = self.residual(x)
        N, C, T, V = x.size()
        # GCN: aggregate over partitions
        x_gcn = self.gcn(x)  # (N, out_ch*n_part, T, V)
        x_gcn = x_gcn.view(N, self.n_part, self.out_ch, T, V)
        x_agg = torch.zeros(N, self.out_ch, T, V, device=x.device)
        for p in range(self.n_part):
            x_agg += torch.einsum("nctv,vw->nctw", x_gcn[:, p], self.A[p])
        # TCN
        x_t = self.tcn(x_agg)
...
class _STGCNOriginalModel(nn.Module):
    """st_gcn_networks 레이어 이름 사용 (1-partition)"""
...
class _STGCNFineTunedModel(nn.Module):
    """layers 레이어 이름 + data_bn 사용 (3-partition)"""
    def __init__(self, num_classes=2):
        super().__init__()
